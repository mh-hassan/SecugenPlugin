...
private void captureB64(CallbackContext callbackContext) {
		
		Log.d(TAG, "Capture x Pressed CAPTURE captureB64");
//    	this.mCheckBoxMatched.setChecked(false);
		byte[] buffer = new byte[mImageWidth*mImageHeight];
		ByteBuffer byteBuf = ByteBuffer.allocate(mImageWidth*mImageHeight);
		dwTimeStart = System.currentTimeMillis();          
...
//            byte[] data = Base64.decode(base64, Base64.DEFAULT);
			...
			long wsq_rslt = sgfplib.WSQGetEncodedImageSize(wsqImageOutSize, SGWSQLib.BITRATE_5_TO_1, buffer, mImageWidth, mImageHeight, 8, 500);
			if(wsq_rslt != SGFDxErrorCode.SGFDX_ERROR_NONE) {
				callbackContext.error(""+result);
				buffer = null;
				return;
			}
			byte[] wsqfinger1ImageOut = new byte[wsqImageOutSize[0]];
			wsq_rslt = sgfplib.WSQEncode(wsqfinger1ImageOut, SGWSQLib.BITRATE_5_TO_1, buffer, mImageWidth, mImageHeight, 8, 500);
			if(wsq_rslt != SGFDxErrorCode.SGFDX_ERROR_NONE) {
				callbackContext.error(""+result);
				buffer = null;
				return;
			...
			//String source = Base64.encodeToString(buffer, Base64.DEFAULT);
			String source = Base64.encodeToString(wsqfinger1ImageOut, Base64.DEFAULT);
			debugMessage("Base64() ret:" + " [" + source + "ms]\n");
...
//            
			...
			sgfplib.GetImageQuality(mImageWidth, mImageHeight, buffer, quality1);
			debugMessage("Base64() Qlty ret:" + " [" + quality1[0] + "]\n");
...
//          
			...
			try {  
				json.put("img", base64); 
				json.put("qlty", quality1[0]); 
				json.put("source", source); 
				json.put("width", mImageWidth);
				json.put("height", mImageHeight);
				json.put("device", mDeviceSN);
//    	        callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK, json));
//    	        
//    	        callbackContext.success(json);
				
				PluginResult p_result = new PluginResult(PluginResult.Status.OK, json);
				p_result.setKeepCallback(true);
				callbackContext.sendPluginResult(p_result);
				
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
//    			callbackContext.success("JSON Error");
				callbackContext.error("101");
			}
			
		}else if(result == SGFDxErrorCode.SGFDX_ERROR_DEVICE_NOT_FOUND){
			
			callbackContext.error(""+result);
//        }else if(result == SGFDxErrorCode.SGFDX_ERROR_WRONG_IMAGE){
//        	
//        	callbackContext.error(""+result);
		}else{
			callbackContext.error(""+result);
...
//        callbackContext.success(base64);
//        callbackContext.success(source);
...
//    						debugMessage("Product Serial: " + device.getSerialNumber() + "\n"); 
						}
						else
...
/**
	...
	public void initDeviceSettings() {
		...
		debugMessage("Setting props: Device: " +mDeviceSN);
